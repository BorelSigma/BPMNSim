...
public class NetworkTopology {
	...
	public static void buildNetworkTopology(String fileName) {
		org.cloudbus.cloudsim.Log.printConcatLine("Topology file: ", fileName);
		...
		try {
			...
			org.cloudbus.cloudsim.Log.printLine("Problem in processing BRITE file. Network simulation is disabled. Error: "
					+ e.getMessage());
		}
			...
			try {
				...
				if (!map.containsKey(cloudSimEntityID)) {
					if (!map.containsValue(briteID)) { // this BRITE node was already mapped?
						...
						org.cloudbus.cloudsim.Log.printConcatLine("Error in network mapping. BRITE node ", briteID, " already in use.");
					}
				} else {
					org.cloudbus.cloudsim.Log.printConcatLine("Error in network mapping. CloudSim entity ", cloudSimEntityID,
							" already mapped.");
				}
			} catch (Exception e) {
				org.cloudbus.cloudsim.Log.printConcatLine("Error in network mapping. CloudSim node ", cloudSimEntityID,
