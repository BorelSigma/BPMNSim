...
public class Switch extends SimEntity {
	...
	public Map<Integer, NetworkHost> hostlist;
         ...
         */
	public List<org.cloudbus.cloudsim.network.datacenter.Switch> uplinkswitches;

        /**
         * List of downlink switches.
         */
	public List<org.cloudbus.cloudsim.network.datacenter.Switch> downlinkswitches;
	...
	protected void processpacket_up(SimEvent ev) {
		...
		if (level == NetworkConstants.EDGE_LEVEL) {
			...
			org.cloudbus.cloudsim.network.datacenter.Switch sw = uplinkswitches.get(0);
		...
		if (level == NetworkConstants.Agg_LEVEL) {
			...
			for (org.cloudbus.cloudsim.network.datacenter.Switch sw : downlinkswitches) {
		...
		if (level == NetworkConstants.ROOT_LEVEL) {
			...
			;
			for (org.cloudbus.cloudsim.network.datacenter.Switch sw : downlinkswitches) {
				for (org.cloudbus.cloudsim.network.datacenter.Switch edge : sw.downlinkswitches) {
